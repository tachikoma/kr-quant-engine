...
- 결과: 524종목 (domestic_equity 304, foreign_investment 220)
- `ETF_LIST` env var가 auto보다 우선 (static 오버라이드)
- `TAXABLE_ETF_TICKERS`도 KRX 분류에서 자동 산출 (240종)
...
기본값 `0`(비활성)으로 보존 중이며, `.env` 미변경로 기존 전략 유지:
...
1. **집중 위험 완화 실험:** 변동성 기반 비중 축소, 상관관계 기반 분산 등 포트폴리오 단위 규칙을 OOS로 검증. 524종목 유니버스에서 diversified 포트폴리오 구성 가능성을 탐색.
2. **524종목 전체 백테스트:** 현재 24종목 캐시 기반 검증 완료. 전체 524종목 pykrx 데이터 수집 후 10년 백테스트 실행 (네트워크 호출 524건, 수 분 소요).
3. **strategy_freeze.json 갱신:** auto 모드 기준 유니버스 스냅샷 생성으로 OOS 추적 정확도 확보.
4. **Phase 1 리스크 시그널 + Phase 2 유니버스 통합 검증:** 멀티 인덱스(`split` 모드)를 524종목 유니버스에서 워크포워드 검증.
5. **진입 주기 유지 + 위험 신호 빈도:** 진입 주기는 20거래일로 두고 위험 신호만 더 자주 확인하는 방식 연구.
