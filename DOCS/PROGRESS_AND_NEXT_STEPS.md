...
1. **집중 위험 완화 실험:** 변동성 기반 비중 축소, 상관관계 기반 분산 등 포트폴리오 단위 규칙을 OOS로 검증.
2. **strategy_freeze.json 갱신:** 현재 정적 16종목 기준 스냅샷 갱신으로 OOS 추적 정확도 확보.
3. **진입 주기 유지 + 위험 신호 빈도:** 진입 주기는 20거래일로 두고 위험 신호만 더 자주 확인하는 방식 연구.
4. **Phase 2 유니버스 재검토 (장기):** 12-24개월 후 새로운 테마 ETF의 검증된 트랙 레코드가 누적되면 재평가. 별도 슬리브(10% 할당)로 분리하여 코어 로테이션 오염 방지.
