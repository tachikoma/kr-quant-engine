...
- HTTP 로그: 키움 어댑터의 HTTP 상세 로그는 `LOG_LEVEL=DEBUG`로 제어합니다 (이전 `KIWOOM_HTTP_DEBUG_RESPONSE`/`BODY`/`BODY_LIMIT` 변수는 제거됨). 재시도 대기: `KIWOOM_HTTP_RETRY_DELAY`, 호출 간격: `KIWOOM_HTTP_MIN_INTERVAL`
...
- 데일리 러너는 `_KRX_HOLIDAYS`(2026년 KRX 공휴일 목록)로 휴장일을 감지하여 자동 중단합니다. 정기 휴장일 외에도 대체공휴일, 근로자의 날, 연말 휴장일(12/31), 그리고 연중 추가로 지정되는 임시공휴일(선거일 등)까지 포함해 갱신이 필요합니다.
